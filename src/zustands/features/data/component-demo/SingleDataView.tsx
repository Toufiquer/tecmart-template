...
import { useUpdateDataMutation, useDeleteDataMutation } from '@/zustands/features/data/useData';
...
const SingleDataView = ({ data }: { data: IData }) => {
  ...
  const [updateData, { isLoading: isLoadingUpdate, isError: isErrorUpdate, isSuccess: isSuccessUpdate, error: errorUpdate }] = useUpdateDataMutation();
  const [deleteData, { isLoading: isLoadingDelete, isError: isErrorDelete, isSuccess: isSuccessDelete, error: errorDelete }] = useDeleteDataMutation();
  const handleUpdate = () => {
    ...
    renderData = (
      <main className="border-1 border-slate-500 p-2 flex gap-2 items-center justify-between">
        ...
        <div className="w-full flex items-center justify-end gap-4">
          ...
          <button className="border-1 rounded-lg border-green-600 text-green-600 px-4 cursor-pointer" onClick={handleUpdate}>
            Update
          </button>{' '}
          <button className="border-1 rounded-lg border-rose-600 text-rose-600 px-4 cursor-pointer" onClick={handleDelete}>
            Delete
          </button>
        </div>
      </main>
