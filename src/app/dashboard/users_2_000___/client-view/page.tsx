...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const url = 'http://localhost:3000/dashboard/users_2_000___/all/api/v1?page=1&limit=4';
  ...
  return (
    <main className="w-full flex flex-col gap-2 p-1 md:p-4">
      {data.map((i: { name: string; _id: string }, idx: number) => (
        <div key={idx + i?.name}>
          <CustomLInk name={i.name} url={`/dashboard/users_2_000___/client-view/details/${i._id}`} />
        </div>
      ))}
    </main>
  );
