...
const ViewUsers_101__Table: React.FC = () => {
  const [sortConfig, setSortConfig] = useState<{ key: keyof IUsers_101__; direction: 'asc' | 'desc' } | null>(null);
  const {
  ...
  const handleSort = (key: keyof IUsers_101__) => {
    setSortConfig(prev => (prev?.key === key ? { key, direction: prev.direction === 'asc' ? 'desc' : 'asc' } : { key, direction: 'asc' }));
  };

  const sortedUsers_101__Data = useMemo(() => {
    if (!sortConfig) return getAllUsers_101__Data;
    return [...getAllUsers_101__Data].sort((a, b) => {
      if (a[sortConfig.key] < b[sortConfig.key]) return sortConfig.direction === 'asc' ? -1 : 1;
      if (a[sortConfig.key] > b[sortConfig.key]) return sortConfig.direction === 'asc' ? 1 : -1;
      return 0;
    });
  }, [getAllUsers_101__Data, sortConfig]);

  const handleSelectAll = (isChecked: boolean) => setBulkData(isChecked ? getAllUsers_101__Data : []);
  const handleSelectRow = (isChecked: boolean, __104_Users__: IUsers_101__) =>
    setBulkData(isChecked ? [...bulkData, __104_Users__] : bulkData.filter(item => item.email !== __104_Users__.email));

  const renderActions = (__104_Users__: IUsers_101__) => (
    ...
    <div className="w-full flex flex-col">
      ...
      <Table className="border-1 border-slate-500">
        <TableHeader className="bg-slate-600 text-slate-50 rounded overflow-hidden border-1 border-slate-600">
          <TableRow>
            <TableHead>
              <Checkbox onCheckedChange={checked => handleSelectAll(!!checked)} checked={bulkData.length === getAllUsers_101__Data.length} />
            </TableHead>
            {['name', 'email', 'passCode', 'alias', 'role', 'createdAt'].map(key => (
              <TableHead
                ...
                onClick={() => handleSort(key as keyof IUsers_101__)}
              >
